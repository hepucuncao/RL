...
def full_attack_training(delete_ratio=0.10):
    ...
    attack_test_x, attack_test_y, test_classes, delete_ratio = train_target_model(
    ...
    attack_train_x, attack_train_y, train_classes, delete_ratio = train_shadow_models(
...
def train_target_model(train_indices, test_indices,
                       ...
                       fc_dim_hidden=50, model='rl', save=True, delete_ratio=0):
    ...
    # 随机删除数据
    if delete_ratio > 0:
        delete_num = int(len(train_indices) * delete_ratio)
        delete_indices = np.random.choice(train_indices, delete_num, replace=False)
        train_indices = np.setdiff1d(train_indices, delete_indices)
        print(f"Deleted {delete_num} examples, remaining: {len(train_indices)}")
    ...
    return attack_x, attack_y, classes, delete_indices  # 添加删除的样本索引

def train_shadow_models(train_indices, test_indices,
                        fc_dim_hidden=50, n_shadow=20, model='rnn',
                        epochs=100, learning_rate=0.05, batch_size=10,
                        save=True, delete_ratio=0):
...
def train_attack_model(data=None,
    ...
    # 返回删除的样本索引，用于记录
    return train_indices, test_indices  # 添加删除的样本索引
...
if __name__ == '__main__':
    ...
    parser.add_argument('--target_model', type=str, default='rl')
    ...
    # attack model
    parser.add_argument('--attack_model', type=str, default='rl')
    ...
    if args.save_data:
        ...
        train_indices, test_indices = full_attack_training(delete_ratio=0.05)
        train_indices, test_indices = full_attack_training(delete_ratio=0.10)
        print("Deleted train indices:", train_indices)
        print("Deleted test indices:", test_indices)
        #full_attack_training(delete_ratio=0)  # 不删除数据，仅用于模型攻击
